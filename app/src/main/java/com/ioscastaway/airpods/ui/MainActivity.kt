...
private fun Screen(vm: MainViewModel = viewModel()) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Pod Signal") }) }) { pad ->
        Column(
            ...
            StatusCard(status, connectedName, monitoring, hasBt,
...
private fun StatusCard(
    status: PodsStatus?, connectedName: String?, monitoring: Boolean, hasBt: Boolean,
        ...
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
            ...
            if (status != null) {
                Row(horizontalArrangement = Arrangement.spacedBy(18.dp)) {
                    Battery("Left", status.leftBattery, status.leftCharging, status.inEarLeft)
                    Battery("Right", status.rightBattery, status.rightCharging, status.inEarRight)
                    Battery("Case", status.caseBattery, status.caseCharging, null)
                }
                Text("Updated ${time(status.timestampMs)} · ${status.rssi} dB", style = MaterialTheme.typography.bodySmall)
            }
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(if (monitoring) "Scanning for beacons" else "Starts by itself when AirPods connect", style = MaterialTheme.typography.bodySmall)
            }
        }
    }
}
